...
function gamesForWeek(data, week) {
  return (data.games || []).filter(game => game.week === week && (data.teams || []).some(team => team.name === game.teamA) && (data.teams || []).some(team => team.name === game.teamB));
}
...
function defaultPickemWeek(data) {
  ...
  return liveOrUpcoming || weeks[weeks.length - 1] || 'Week 1';
}
