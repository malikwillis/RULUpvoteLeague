...
loadLeagueData().then(originalData => {
  ...
  pageHeader(data, 'score-entry.html', 'Commissioner', 'Score Entry', 'Select players, enter upvotes, and publish to the site');
  pageFooter(data);
  ...
  root.innerHTML = `
    <section class="card">
      ...
      <p class="note">Use Publish To Website to commit the update to GitHub so everyone sees it after Vercel redeploys.</p>
    </section>
    ...
    <section class="card">
      ...
      <button class="btn" id="publishBtn">Publish To Website</button>
      <button class="btn" id="downloadBtn">Download Backup static-data.js</button>
      <button class="btn" id="copyBtn">Copy Backup static-data.js</button>
      <button class="btn" id="clearPreviewBtn">Clear Local Preview</button>

      <label class="label" style="margin-top:14px">Admin Code</label>
      <input id="adminCode" type="password" placeholder="Required for Publish To Website">
    ...
    <section class="card">
      ...
      <div class="row"><span>Apply Score To Data</span><strong>Updates preview on this device</strong></div>
      <div class="row"><span>Publish To Website</span><strong>Updates GitHub static-data.js</strong></div>
      <div class="row"><span>Vercel</span><strong>Redeploys automatically</strong></div>
      <div class="row"><span>Everyone sees it</span><strong>After redeploy finishes</strong></div>
    </section>
  `;
  ...
  $('#publishBtn').addEventListener('click', publishToWebsite);
  ...
  function applyScoreToData() {
    ...
    $('#exportStatus').innerHTML = `
      <div class="card" style="margin-top:12px">
        ...
        <p class="muted">Local preview updated. Click Publish To Website so everyone can see it.</p>
      </div>
    `;
  ...
  async function publishToWebsite() {
    if (!applyScoreToData()) return;

    const adminCode = $('#adminCode').value.trim();
    if (!adminCode) {
      alert('Enter admin code before publishing.');
      return;
    }
    ...
    $('#exportStatus').innerHTML = `
      <div class="card" style="margin-top:12px">
        <h2>Publishing...</h2>
        <p class="muted">Updating static-data.js on GitHub. Vercel will redeploy after the commit.</p>
      </div>
    `;
    ...
    try {
      const response = await fetch('/api/save-data', {
        ...
        body: JSON.stringify({ adminCode, data: workingData })
      });

      const result = await response.json();

      if (!response.ok || !result.ok) {
        throw new Error((result.errors || ['Publish failed.']).join(' '));
      }

      $('#exportStatus').innerHTML = `
        <div class="card" style="margin-top:12px">
          <h2>Published To GitHub</h2>
          <p class="note">Vercel should redeploy automatically. Refresh the public site after the deployment finishes.</p>
          ${result.commit ? `<p><a class="btn" href="${escapeHtml(result.commit)}" target="_blank" rel="noopener">View Commit</a></p>` : ''}
        </div>
      `;
    } catch (error) {
      $('#exportStatus').innerHTML = `
        <div class="card" style="margin-top:12px">
          ...
          <p class="muted">${escapeHtml(error.message || 'Could not publish to GitHub.')}</p>
          <p class="muted">Use Download Backup static-data.js as a fallback.</p>
        </div>
      `;
    }
  }
