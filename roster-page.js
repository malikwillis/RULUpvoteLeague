loadLeagueData().then(data => {
  ...
  function rosterUpvoteSum(team) {
    return (team.roster || []).reduce((sum, player) => sum + Number(player.upvotes || 0), 0);
  }
  ...
  function renderRoster(term = '') {
    ...
    const cards = (data.teams || []).map(team => {
      ...
      const rosterSum = rosterUpvoteSum(team);
        ...
        <article class="card roster-card">
          <div class="roster-head">
            <div>
              ...
              <p class="muted">${escapeHtml(team.record || '0-0')} · ${escapeHtml(team.conferenceRecord || '0-0')} conf</p>
            </div>
          ...
          <div class="grid two">
            <div class="mini-card">
              ...
              <p class="muted">${sched.gamesPlayed} games · ${fmt(sched.averageUpvotes)} avg</p>
            </div>

            <div class="mini-card">
              <div class="label">Current Roster Sum</div>
              <div class="kpi">${fmt(rosterSum)}</div>
              <p class="muted">Current players only</p>
            </div>
          </div>
          ...
          <div class="mini-card">
            ...
            <div class="row">
              <span>Official Locked Total</span>
              <strong>${fmt(team.totalUpvotes)}</strong>
            </div>
          </div>
